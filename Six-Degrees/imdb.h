...
class imdb {
 ...
 private:
  ...
  static int cmp_fun_actors(const void * s1, const void * s2);
  static int cmp_fun_movies(const void * s1 , const void *s2);
  bool process_actor_records(char* records_ptr, const string& player, vector<film>& films) const; 
  bool extract_movie_details(char* records_ptr, short number_of_movies, vector<film>& films) const ;
