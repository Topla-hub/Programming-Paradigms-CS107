 ...
 */
string generator(const string& nonterminal, map<string, Definition>& grammar)
{
    ...
    const Production& prod = it->second.getRandomProduction();
    string result;

    for (const string& word : prod) {
        if (word.front() == '<' && word.back() == '>') {
            result += generator(word, grammar);
        } else {
            result += word + " ";
        }
    }
...
int main(int argc, char *argv[])
{
  ...
  srand(static_cast<unsigned int>(time(nullptr)));
    ...
    for (int i = 0; i < 3; ++i) 
    {
        ...
        string sentence = generator("<start>", grammar); 
