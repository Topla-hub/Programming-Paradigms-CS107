...
;;(intersection-points) 
(define (intersect-all circle lst)
  (apply append (map (lambda (x) (intersect circle x)) lst)))
(define (helper-for-intersect lst)
  (if (null? lst)
      '()
      (append (intersect-all (car lst) (cdr lst)) (helper-for-intersect (cdr lst)))))
(define (intersection-points lst)
  (helper-for-intersect lst))


;;(distance-product)
(define (helper-for-dis point1  point2)
  (if 
    (equal? point1 point2) 1
    (dist point1 point2)
  )
)
(define (distance-product point lst)
  (apply * (map (lambda (x) (helper-for-dis point x)) lst))
)


;;(rate-points)
(define (rate-points lst)
  (map (lambda (x) (reverse (cons x (list (distance-product x lst))))) lst)
)
;;(sort-points)
...
;;(clumped-points)
(define (helper-for-clump lst)
  (if
    (null? lst) '()
    (append (cdr (car lst)) (helper-for-clump (cdr lst)))
  )
)
(define (clumped-points lst)
  (prefix-of-list (helper-for-clump (quicksort (rate-points lst))) 
    (div (length (helper-for-clump (quicksort (rate-points lst)))) 2)
  )
)
;;(average-point)
(define (sumy lst)
...
(define (average-point lst)
 (let ((avg-pt (list (avgx lst) (avgy lst))))
    (list (distance-product avg-pt lst) avg-pt)
  )
) 
...
(define (where-am-i lst1 lst2)
  (quicksort (map best-estimate (all-guesses lst1 lst2)))
)
